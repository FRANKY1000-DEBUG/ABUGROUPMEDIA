    ...
    <div className="flex min-h-screen">
      ...
      <div className="min-w-0 flex-1 p-8">
        <div className="mb-7 flex items-center justify-between">
          <div>
            <h1 className="text-xl font-extrabold">Émissions</h1>
            <p className="mt-0.5 text-[13px] text-muted">{shows.length} émission(s)</p>
          </div>
          <a href="/admin/emissions/nouveau" className="rounded-[10px] bg-ink px-5 py-2.5 text-[13.5px] font-bold text-bg">
            + Nouvelle émission
          </a>
        </div>
          ...
          {shows.map((show) => (
            <div key={show.id} className="rounded-2xl border border-line bg-surface p-5">
              ...
              <div className="flex items-center gap-2">
                <a href={`/admin/emissions/${show.id}`} className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-line py-2 text-[12.5px] font-semibold text-muted hover:text-ink">
