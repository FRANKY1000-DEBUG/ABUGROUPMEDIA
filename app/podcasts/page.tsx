...
// Modèle de page catégorie, dupliqué depuis /reportages (voir la note dans
// ce dossier-là si tu ajoutes encore une catégorie).
const CATEGORY_SLUG = "podcasts";
const CATEGORY_LABEL = "Podcasts";
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="border-b border-line px-4 pb-8 pt-8 md:px-10">
          ...
          <p className="max-w-[520px] text-[14.5px] leading-relaxed text-muted">
            Discussions, interviews et émissions audio d&apos;AGM à écouter ou regarder.
          </p>
        </div>
        ...
        <div className="flex items-center justify-between px-4 pb-4 pt-6 md:px-10">
          <span className="text-[13px] text-muted">{list.length} podcasts</span>
        </div>
        <div className="grid grid-cols-2 gap-5 px-4 pb-16 sm:grid-cols-3 md:grid-cols-4 md:px-10">
          ...
          {list.length === 0 && (
            <p className="col-span-full text-sm text-muted">Aucun podcast publié pour le moment.</p>
          )}
        </div>
      </div>
    </div>
